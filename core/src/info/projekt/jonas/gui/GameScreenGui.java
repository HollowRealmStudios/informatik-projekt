...
public class GameScreenGui extends Gui {
	...
	public boolean touchDown(int screenX, int screenY, int pointer, int button) {
		...
		if (button == Input.Buttons.LEFT && !guiOpen) {
			...
			return true;
		} else if (!selectedRoom.equals("") && getSelectedRoom() == null) {
			if (GAME_STORAGE.currency.get() >= Objects.requireNonNull(Registry.getRoom(selectedRoom)).getCost()) {
				try {
			...
			else notification.show("Not enough money", 2);
		}
		return false;
	}
	...
	public boolean keyDown(int keycode) {
		switch (keycode) {
			...
			case CONTROL_RIGHT:
				...
				break;
			case ESCAPE:
				RenderUtils.hideAllGuisExcept(GameScreenGui.class);
			...
			public boolean keyTyped(InputEvent event, char character) {
				switch (field.getText().toLowerCase()) {
					...
					case "items":
						...
						field.setVisible(false);
						guiOpen = false;
						GuiProvider.requestGui(ItemListGui.class).show(GAME_STORAGE.ITEMS);
						break;
					case "components":
						field.setText("");
						field.setVisible(false);
						guiOpen = false;
						GuiProvider.requestGui(ItemListGui.class).show(GAME_STORAGE.COMPONENTS);
						break;
					case "new_item":
						field.setText("");
						field.setVisible(false);
					...
					case "test":
						...
						field.setVisible(false);
						guiOpen = false;
						notification.show("Test String", 5);
						break;
				}
				return true;
			}
	...
	private void updateGui() {
		currency.setText(click);
		food.setText("Food: " + GAME_STORAGE.food.get());
		water.setText("Water: " + GAME_STORAGE.water.get());
		energy.setText("Energy: " + GAME_STORAGE.energy.get());
	}
