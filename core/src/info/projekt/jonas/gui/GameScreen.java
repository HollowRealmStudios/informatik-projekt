...
public class GameScreen extends InputAdapter implements Screen {
    ...
    public boolean scrolled(int amount) {
        InfoProjekt.manager.setZoom(InfoProjekt.manager.getZoom() + (amount > 0 ? 0.2f : -0.2f));
        return false;
    }
    ...
    private void handleMoveKeys() {
        if (Gdx.input.isKeyPressed(Input.Keys.W))
            InfoProjekt.manager.translateRelative(new Vector2(0, Gdx.input.isKeyPressed(Input.Keys.SHIFT_LEFT) ? 20 : 10));
        else if (Gdx.input.isKeyPressed(Input.Keys.S))
            InfoProjekt.manager.translateRelative(new Vector2(0, Gdx.input.isKeyPressed(Input.Keys.SHIFT_LEFT) ? -20 : -10));
        if (Gdx.input.isKeyPressed(Input.Keys.A))
            InfoProjekt.manager.translateRelative(new Vector2(Gdx.input.isKeyPressed(Input.Keys.SHIFT_LEFT) ? -20 : -10, 0));
    ...
    private Room getSelectedRoom() {
        Vector3 pos = InfoProjekt.manager.getCamera().unproject(new Vector3(Gdx.input.getX(), Gdx.input.getY(), 0));
